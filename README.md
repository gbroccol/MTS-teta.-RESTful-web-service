...
![books](https://github.com/gbroccol/RESTful-Web-Service/blob/main/tutorial_img/02.png)
...
Как Spring Boot понимает какой бин создавать? Для этого мы используем аннотации, в простом виде **@Component**, **@Repository** или **@Service**. 

* **@Component** - общий стереотип для любого компонента, управляемого Spring.


* **@Repository** - Это указывает на то, что класс определяет хранилище данных. маркером для любого класса, который выполняет роль или стереотип (также известный как объект доступа к данным или DAO) хранилища. Среди применений этого маркера - автоматический перевод исключений. 
* **@Service** - содержат бизнес-логику и вызывают методы на уровне хранилища.



Чуть сложнее — **@Controller** и **@RestController**. Дальше в уроках мы будем много их использовать поэтому пару слов про разницу между этими анотациями:

* **@Controller** Диспетчер просматривает классы, аннотированные @Controller и обнаруживает в них аннотации @RequestMapping. Мы можем использовать @RequestMapping только в тех классах, которые аннотированы @Controller.
